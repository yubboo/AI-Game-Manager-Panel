...
try{
  ...
  foreach($entry in @(
    @{Name='AGMP-GitHub.bat';Token='%~dp0push-agmp.ps1'},
    @{Name='AGMP-Sync.bat';Token='%~dp0sync-agmp.ps1'}
  )){
    $helperPath=Join-Path $root $entry.Name
    if(-not (Test-Path -LiteralPath $helperPath -PathType Leaf)){Fail "缺少根辅助入口：$($entry.Name)";continue}
    $helperBytes=[IO.File]::ReadAllBytes($helperPath)
    if($helperBytes | Where-Object {$_ -gt 127}){Fail "$($entry.Name) 必须 ASCII-safe。"}
    if($helperBytes.Length -ge 3 -and $helperBytes[0]-eq 0xEF -and $helperBytes[1]-eq 0xBB -and $helperBytes[2]-eq 0xBF){Fail "$($entry.Name) 禁止 UTF-8 BOM。"}
    $helperText=[Text.Encoding]::ASCII.GetString($helperBytes)
    if(-not $helperText.Contains($entry.Token)){Fail "$($entry.Name) 没有正确定位对应 PowerShell。"}
    if(($helperText -replace "`r`n",'').Contains("`n")){Fail "$($entry.Name) 不是 CRLF。"}
  }
  foreach($entry in @('push-agmp.ps1','sync-agmp.ps1')){
    $helperPath=Join-Path $root $entry
    if(-not (Test-Path -LiteralPath $helperPath -PathType Leaf)){Fail "缺少根 PowerShell 助手：$entry";continue}
    $helperBytes=[IO.File]::ReadAllBytes($helperPath)
    if($helperBytes.Length -lt 3 -or $helperBytes[0]-ne 0xEF -or $helperBytes[1]-ne 0xBB -or $helperBytes[2]-ne 0xBF){Fail "$entry 缺少 UTF-8 BOM。"}
    $helperText=[IO.File]::ReadAllText($helperPath)
    if(($helperText -replace "`r`n",'').Contains("`n")){Fail "$entry 不是 CRLF。"}
  }
