...
try {
  ...
  if (version) {
    ...
    if (!fs.existsSync(path.join(root, 'docs/releases', `${version}.md`))) failures.push(`缺少 docs/releases/${version}.md`)
  }
...
try {
  const prompt = 'docs/prompts/0.1.64-ten-issue-consolidated-fix.md'
  if (!fs.existsSync(path.join(root, prompt))) failures.push(`0.1.64 缺少正式提示词归档：${prompt}`)
