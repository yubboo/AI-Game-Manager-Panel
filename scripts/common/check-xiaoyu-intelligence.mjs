...
try {
  ...
  requireCheck(loop.includes('Intelligence IntelligenceContext') && loop.includes('WithIntelligence'), '每个 Rust Brain Frame 必须能接收 Host 过滤后的 Intelligence Context')
  ...
  requireCheck(rust.includes('不要在每个普通步骤后反问') && rust.includes('is_procedural_wait') && rust.includes('agmp.capability.search'), 'Rust Brain 必须拒绝程序性 wait，并在能力未知时先发现 Capability')
  ...
  requireCheck(rust.includes('frame.intelligence') && rust.includes('Host Tool Contract') && rust.includes('Expert / Skill > Memory'), 'Rust XiaoYu Brain 必须明确 Intelligence 不可信优先级和 Host 最终权威')
  requireCheck(rust.includes('memory.remember') && rust.includes('不要保存秘密'), 'Rust Brain 必须约束主动记忆行为')
