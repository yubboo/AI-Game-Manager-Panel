...
0.2.19 已把 Linux PTY / Windows ConPTY 冻结为 CI-proven Runtime primitive。0.2.20 只改变**授权后的执行路由**：`model decision → Go Host Tool contract → identity/RBAC/step-up/approval → server-owned Run invocation context → Rust terminal/start → output/status → close → Host observation/verification`。

Go Host 仍是唯一授权权威，Rust Terminal 只接收内部 `HostAuthorized=true`。Agent 路径不允许在 Native Terminal 失败后自动再跑 legacy shell；人工/兼容命令仍留在 `platform/runtime`，因此迁移期间同一动作只有一个执行 Runtime。

## 0.2.19 Windows ConPTY Input Pipe Convergence

0.2.19 不改变 Terminal contract，只修 Windows `CreateProcessW` 的 stdio 归属。ConPTY child 的 `STARTUPINFOEXW` 必须设置 `STARTF_USESTDHANDLES`，且三个标准句柄保持 NULL，防止 CI/test harness 的 redirected handles 被 Windows 自动复制给 console child。这样 `cmd.exe` 的输入输出只能通过 ConPTY pipes 进入 Rust Terminal Runtime；Go Host 的身份/RBAC/审批权威不变。

## 0.2.18 Windows ConPTY Input Convergence

0.2.17 已证明 Windows ConPTY 的 ABI、`CreateProcessW` cwd、进程启动与 resize 路径可以进入真实 Windows Runner；剩余失败是输入层。0.2.18 把 Windows Enter 语义固定为单个 CR（`\r`），Linux PTY / fallback 仍为 LF。
...
Persistent XiaoYu Rust Worker
        │
        ├─ Linux → native PTY      ✅ CI-proven / LF
        └─ Windows → ConPTY
             ├─ Win32 cwd normalize ✅
             ├─ Enter = CR           → 0.2.18
             └─ resize/output         ✅ startup path proven
```

该差异只存在 Rust Terminal backend；`terminal/*` contract、Go Host identity/RBAC/approval 与模型可见权限均不改变。
