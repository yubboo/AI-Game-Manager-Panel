...
const form = reactive<XiaoYuSaveModelRequest>({
  name: '我的模型', provider: 'openai', protocol: 'openai-responses', baseUrl: 'https://api.openai.com/v1', model: '', apiKey: '', enabled: true,
...
const apiKeyHint = computed(() => {
  ...
  if (selectedPreset.value?.apiKeyOptional) return '本地服务通常可留空'
  return '请输入 API Key'
})
...
function resetForm() {
  Object.assign(form, {
    id: undefined, name: '我的模型', provider: 'openai', protocol: 'openai-responses', baseUrl: 'https://api.openai.com/v1', model: '', apiKey: '', enabled: true,
    contextWindow: 128000, maxOutputTokens: 8192, thinkingMode: 'default', reasoningEffort: 'default', extra: {},
  })
  extraText.value = '{}'
  testResult.value = null
...
function edit(profile: XiaoYuModelProfile) {
  Object.assign(form, {
    id: profile.id, name: profile.name, provider: profile.provider, protocol: profile.protocol, baseUrl: profile.baseUrl, model: profile.model, apiKey: '', enabled: profile.enabled,
...
function connectionRequest(): XiaoYuModelConnectionRequest {
  return { id: form.id, provider: form.provider, protocol: form.protocol, baseUrl: form.baseUrl, model: form.model, apiKey: form.apiKey || undefined, thinkingMode: form.thinkingMode, reasoningEffort: form.reasoningEffort, extra: parseExtra() }
}
...
async function save() {
  ...
  try {
    ...
    edit(value)
    form.apiKey = ''
    message.value = value.isDefault ? '模型已保存，小鱼正在使用这个大脑。' : '模型已保存。你可以把它设为“小鱼默认大脑”。'
...
async function remove(profile: XiaoYuModelProfile) {
  if (!canManage.value || !window.confirm(`确定删除模型“${profile.name}”吗？对应 API Key 也会从本机 Secret Vault 删除。`)) return
...
<template>
  <section class="settings-section model-manager">
    <div class="settings-section__intro">
      ...
      <div><strong>模型管理 · 小鱼大脑</strong><span>小鱼必须先配置一个可用的大模型才能进入自主工作状态。模型只负责思考，所有真实操作仍经过 AGMP Host、权限/审批和结构化 Tool。</span></div>
    </div>
    ...
    <div class="settings-subsection">
      <div class="settings-subsection__heading"><strong>选择模型服务</strong><span>官方接口、本地部署、OpenAI-Compatible 中转站和自定义地址使用同一套模型管理入口。</span></div>
    ...
    <div class="model-editor-grid">
      <div class="model-editor-card">
        <div class="settings-subsection__heading"><strong>{{ editing ? '编辑模型' : '添加模型' }}</strong><span>API Key 与普通设置分离保存，页面不会回显完整密钥。</span></div>
        <div class="form-grid two">
          ...
          <label class="field-block"><span class="feature-title">模型代码</span><div class="field-action"><input v-model="form.model" :disabled="!canManage" list="xiaoyu-model-list" placeholder="例如：gpt-5 / deepseek-chat" /><button type="button" :disabled="!canManage || discovering" @click="discover">{{ discovering ? '拉取中' : '拉取' }}</button></div><datalist id="xiaoyu-model-list"><option v-for="item in discoveredModels" :key="item" :value="item" /></datalist></label>
          <label class="field-block"><span class="feature-title">接口协议</span><select v-model="form.protocol" :disabled="!canManage"><option value="openai-responses">OpenAI Responses（原生）</option><option value="deepseek">DeepSeek（原生）</option><option value="anthropic">Anthropic Messages（原生）</option><option value="gemini">Google Gemini（原生）</option><option value="openai-compatible">OpenAI Compatible</option></select></label>
          ...
          <label class="field-block span-two"><span class="feature-title">Base URL</span><input v-model="form.baseUrl" :disabled="!canManage" placeholder="https://api.example.com/v1" /></label>
          <label class="field-block"><span class="feature-title">API Key</span><input v-model="form.apiKey" :disabled="!canManage" type="password" autocomplete="new-password" :placeholder="apiKeyHint" /><small>{{ apiKeyHint }}</small></label>
      ...
      <aside class="connection-card">
        <div class="settings-subsection__heading"><strong>连接测试</strong><span>测试不再只看 /models：填写模型代码后会追加一次带 XiaoYu Tool Schema 的最小实际请求，确认该 Provider 真能给小鱼调用。API Key 不会写入日志或 Trace。</span></div>
        ...
        <dl><div><dt>请求地址</dt><dd>{{ testResult?.endpoint || form.baseUrl || '-' }}</dd></div><div><dt>耗时</dt><dd>{{ testResult?.latencyMs ? `${testResult.latencyMs} ms` : '-' }}</dd></div><div><dt>发现模型</dt><dd>{{ testResult?.models?.length ?? '-' }}</dd></div><div><dt>密钥</dt><dd>{{ apiKeyHint }}</dd></div></dl>
      </aside>
    </div>
    ...
    <div class="settings-subsection models-list-section">
      ...
      <div v-else class="model-table-wrap"><table class="model-table"><thead><tr><th>模型</th><th>服务商 / 协议</th><th>接口</th><th>状态</th><th>小鱼大脑</th><th>操作</th></tr></thead><tbody><tr v-for="profile in catalog?.profiles ?? []" :key="profile.id"><td><strong>{{ profile.name }}</strong><small>{{ profile.model }}</small></td><td><span>{{ profile.provider }}</span><small>{{ profile.protocol }}</small><div class="capability-row compact"><span v-for="item in capabilityLabels(profile.capabilities)" :key="item" class="capability-pill">{{ item }}</span></div></td><td><code>{{ profile.baseUrl }}</code></td><td><span :class="['model-status', { enabled: profile.enabled }]">{{ profile.enabled ? '启用' : '禁用' }}</span><small>{{ profile.lastTestAt ? (profile.lastTestOk ? '连接测试通过' : '最近测试失败') : (profile.hasApiKey ? '密钥已保存 · 未测试' : (catalog?.presets.find(item => item.id === profile.provider)?.apiKeyOptional ? '无需密钥 · 未测试' : '缺少密钥')) }}</small></td><td><span v-if="profile.isDefault" class="default-pill">默认大脑</span><button v-else type="button" class="link-button" :disabled="!canManage || !profile.enabled" @click="setDefault(profile)">设为默认</button></td><td><div class="table-actions"><button type="button" @click="edit(profile)">编辑</button><button type="button" :disabled="!canManage" @click="remove(profile)">删除</button></div></td></tr></tbody></table></div>
    </div>

    <div class="settings-footer"><span class="save-message" :class="{ active: message }">{{ message || '模型配置完成后，小鱼 Harness 会自动读取默认大脑，不需要重启 AGMP。' }}</span><button class="btn btn--secondary" type="button" @click="refresh">刷新</button></div>
  </section>
</template>
