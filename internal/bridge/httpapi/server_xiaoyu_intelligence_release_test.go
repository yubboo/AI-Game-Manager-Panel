...
func TestXiaoYuIntelligenceRequiresOfficialLicenseInReleaseBuild(t *testing.T) {
	...
	recorder := request(handler, http.MethodGet, "/api/v1/xiaoyu/intelligence", nil, owner.Token)
	if recorder.Code == http.StatusOK || !strings.Contains(recorder.Body.String(), "许可证") {
		t.Fatalf("release XiaoYu intelligence must fail closed without official license: status=%d body=%s", recorder.Code, recorder.Body.String())
	}
}
