...
func builtinSkills() []SkillDefinition {
	return []SkillDefinition{
		...
		{ID: "builtin.skill.game-server-one-click-deploy", Name: "游戏服务器一键部署", Description: "从用户的一句话目标出发，自动完成预检、依赖、安装、配置、启动、验证与安全恢复的通用开服流程。", Prompt: "用户要求‘开服/部署服务器/把服务器弄好’时，把最终可用状态作为目标，而不是只做安装。先识别游戏与实例，再做 Preflight（运行环境、SteamCMD/Java、磁盘、端口、现有数据）；已有实例先保护数据与配置。安装/更新后生成或校验实例配置，启动真实进程，并用状态、日志、端口、游戏就绪信号验证。未 Ready 时进入诊断/恢复循环。只有真实 Ready 才 complete。需要 Token/密码等用户独占凭据时才 wait；需要风险审批时交给 Host Approval。", Tags: []string{"deploy", "server", "one-click", "steamcmd", "java", "dst", "minecraft", "开服", "一键部署", "服务器", "部署"}, ToolAllowlist: []string{"agmp.capability.search", "system.info", "environment.status", "environment.catalog", "environment.game_profile", "environment.install_java", "environment.install_steamcmd", "games.list", "game.deploy.plan", "game.deploy", "game.instance.list", "backup.list"}, Checklist: []string{"识别目标游戏与实例", "执行环境/依赖/磁盘/端口预检", "保护已有存档与配置", "补齐 SteamCMD/Java 等依赖", "安装或更新服务端", "创建/校验实例配置", "启动服务器", "读取真实状态/日志/端口", "未 Ready 则诊断并安全恢复", "Ready 后再完成"}, Validators: []string{"不能把下载完成当成开服完成", "不能只看到进程存在就判断游戏 Ready", "变更已有实例前优先确保可恢复性", "失败后先诊断并尝试安全恢复，不能立刻把步骤甩回用户"}, Enabled: true, Visibility: VisibilityOrganization, Builtin: true},
		...
		{ID: "builtin.skill.minecraft-operations", Name: "Minecraft 运维", Description: "Minecraft Java 版本、服务端与日志运维。", Prompt: "Minecraft 开服必须先调用 game.deploy.plan 实时查证 Mojang/Paper/Fabric 事实，再通过同一 game.deploy 内核执行。普通正版开服若用户没有提离线/混合认证，使用安全默认 onlineMode=true；用户明确说离线/混合或账号模式存在歧义时必须先澄清，不能静默关闭正版验证。eulaAccepted 只有在用户明确接受 Minecraft EULA 后才能设为 true。用户说‘开服’默认 startAfterDeploy=true；只有明确要求‘只下载/只准备’时才设为 false。下载、进程存在都不算完成，必须同时看到 Done Ready marker 并通过 Minecraft 协议 Ping。任何升级必须保护 world/config/plugin 数据并验证启动日志。", Tags: []string{"minecraft", "java"}, GameIDs: []string{"minecraft.java", "minecraft"}, ToolAllowlist: []string{"game.deploy.plan", "game.deploy", "game.instance.list", "environment.game_profile", "environment.install_java"}, Checklist: []string{"先 game.deploy.plan 实时查证 MC/服务端版本", "匹配 Java Runtime", "明确 EULA 与账号模式", "通过 game.deploy 部署并校验服务端", "等待 Done + Minecraft Ping"}, Validators: []string{"不得凭模型记忆猜版本或 Java", "不得替用户接受 EULA", "离线/混合认证不得静默推断", "只有 Done + Minecraft Ping 才能宣告开服完成"}, Enabled: true, Visibility: VisibilityOrganization, Builtin: true},
...
func builtinExperts() []ExpertDefinition {
	return []ExpertDefinition{
		...
		{ID: "builtin.expert.minecraft", Name: "Minecraft 专家", Description: "Minecraft Java/Paper/Folia/插件/世界与服务端部署专家。", Prompt: "先确认 Minecraft 与服务端实现版本，再决定 Java、启动参数和插件兼容策略。未知插件行为必须观察实际日志/配置，不靠猜测。", Domains: []string{"Minecraft", "Paper", "Folia", "Java", "Plugins"}, GameIDs: []string{"minecraft.java", "minecraft"}, Knowledge: []string{"Java Runtime 兼容性必须以 Mojang 当前版本元数据为准", "Paper/Fabric 构建必须实时查询各自官方上游", "世界、插件和配置属于需要优先保护的数据"}, SkillIDs: []string{"builtin.skill.game-server-one-click-deploy", "builtin.skill.minecraft-operations", "builtin.skill.safe-change", "builtin.skill.backup-recovery"}, Validators: []string{"启动完成需要真实日志/进程/端口证据"}, RecoveryRules: []string{"升级失败优先回滚 jar/config/plugin 与 world 备份，不直接删除世界"}, Enabled: true, Visibility: VisibilityOrganization, Builtin: true},
